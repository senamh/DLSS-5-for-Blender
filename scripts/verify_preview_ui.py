...
def tick():
    ...
    schedule=[(17,117,'on'),(20,121,'toggle-off'),(25,117,'off'),
              (28,121,'toggle-on'),(33,117,'on-again'),(35,119,'menu-open'),
              (37,119,'menu-close')]
    for seconds,vk,label in schedule:
        if elapsed>seconds and label not in done:
            if label=='menu-open':
                events.append({'before_menu_distance':region.view_distance,
                    'before_menu_rotation':list(region.view_rotation)})
            key(vk,label)
            done.add(label)
    if elapsed>39 and 'rotate' not in done:
        before=next(e for e in events if 'before_menu_distance' in e)
        events.append({'menu_preserves_view':abs(region.view_distance-before['before_menu_distance'])<1e-6
            and max(abs(a-b) for a,b in zip(region.view_rotation,before['before_menu_rotation']))<1e-6})
        region.view_perspective='PERSP'
        region.view_rotation=Quaternion((0,0,1),.15)@region.view_rotation
        done.add('rotate')
    if elapsed>44 and 'rotated' not in done:
        key(117,'rotated')
        done.add('rotated')
    if elapsed>46:
        (out/'ui.json').write_text(json.dumps({'project':str(project),'saved':False,'ticks':ticks,
            ...
            'scene_engine':scene.render.engine,'cycles_device':scene.cycles.device},indent=2))
